...
juno-local:
	...
	docker run --rm -d --name cosmwasm \
		...
		ghcr.io/cosmoscontracts/juno:v10.1.0 /opt/setup_and_run.sh {{test_addrs}}
...
non-ci-integrated: download-deps optimize juno-local
	RUST_LOG=info cargo run --bin non-ci-integrated-tests